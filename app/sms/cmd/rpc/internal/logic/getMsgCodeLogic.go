...
func (l *GetMsgCodeLogic) GetMsgCode(in *pb.GetMsgCodeReq) (*pb.GetMsgCodeResp, error) {
	...
	// 将验证码和手机号保存到redis中
	l.svcCtx.RedisClient.SetexCtx(l.ctx, fmt.Sprintf(globalkey.CacheSmsPhoneKey, in.PhoneNum), randCode, globalkey.CacheSmsPhoneKeyExp)
...
func (l *GetMsgCodeLogic) selectSmsClient(option string, in *pb.GetMsgCodeReq) (SmsClient, string) {
	...
	switch option {
	case "tencent":
		templateSet := []string{randCode, strconv.Itoa(globalkey.CacheSmsPhoneKeyExp)}
		logx.Info(l.svcCtx.Config.SmsConfig)
